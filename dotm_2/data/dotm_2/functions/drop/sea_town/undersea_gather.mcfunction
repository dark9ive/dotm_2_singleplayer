tellraw @s[tag=show_kill] [{"text":"已擊殺 ","color":"gray"},{"text":"海底聚能者","color":"yellow","bold":true},{"text":" !","color":"gray"}]
advancement revoke @s only dotm_2:drop/sea_town/undersea_gather
...
execute if score @s level matches 2100.. run give @s minecraft:prismarine_shard{CustomModelData:1,HideFlags:1,display:{Name:"[{\"text\":\"聚能者碎片\",\"color\":\"dark_gray\",\"bold\":true}]",Lore:["\"\"","\"§7從海底聚能者身上掉落的\"","\"§7碎片，似乎可用來製造東西\"","\"\"","\"\"","\"§c§L取得方式 : §7擊殺海底聚能者\""]},Enchantments:[{id:"minecraft:unbreaking",lvl:1}]} 1
