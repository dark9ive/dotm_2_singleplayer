tellraw @s[tag=show_kill] [{"text":"已擊殺 ","color":"gray"},{"text":"初階狙擊手","color":"green","bold":true},{"text":" !","color":"gray"}]
scoreboard players add @s pay_exp_hp 12
scoreboard players add @s pay_exp_atk 2
scoreboard players add @s pay_money_hp 12
...
advancement revoke @s only dotm_2:drop/home/tiro_sniper
...
execute if score @s item_buff matches ..0 run function dotm_2:main/other/random
execute if score @s item_buff matches ..0 if score #system random <= #system material_1_1 run scoreboard players add @s material_1_1 1
function dotm_2:drop/equipment/normal/guha
