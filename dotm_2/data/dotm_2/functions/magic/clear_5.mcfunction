...
tellraw @a[tag=show_skill,distance=..16] [{"selector":"@s","bold":true,"color":"red"},{"text":" 施展了 ","bold":true,"color":"gray"},{"text":"淨化","color":"dark_purple","bold":true,"hoverEvent":{"action":"show_text","value":{"text":"","extra":[{"text":"移除 8 公尺內所有友軍負面效果","bold":true}]}}},{"text":"(Lv:5)","color":"aqua","bold":false}]
scoreboard players set @s clear_cd -1200
