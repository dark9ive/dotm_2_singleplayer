...
execute if score @s register < #system register run scoreboard players operation #system register = @s register
scoreboard players operation #share_money register = @s pay_money_hp
...
execute at @s as @a[distance=0.01..16] run scoreboard players operation @s money += #share_money register
execute at @s run tellraw @a[distance=0.01..16,tag=show_money] [{"text":"已從","color":"gray"},{"selector":"@s"},{"text":"獲得 ","color":"gray"},{"score":{"name":"#share_money","objective":"register"}},{"text":" 賞金分成。","color":"gray"}]
