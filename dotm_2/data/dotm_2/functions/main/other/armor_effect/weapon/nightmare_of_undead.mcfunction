...
tag @e[type=#dotm_2:death_mob,nbt={HurtTime:10s},sort=nearest,limit=1,distance=..50] add target
data modify entity @e[type=#dotm_2:all_mob,nbt={HurtTime:10s},sort=nearest,limit=1,distance=..50] HurtTime set value 0s
execute if entity @e[tag=target] if data entity @s SelectedItem.tag.Parameter[{"name":"skill_level","value":1}] run scoreboard players set @s cause_damage 1000
execute if entity @e[tag=target] if data entity @s SelectedItem.tag.Parameter[{"name":"skill_level","value":2}] run scoreboard players set @s cause_damage 4000
execute if entity @e[tag=target] if data entity @s SelectedItem.tag.Parameter[{"name":"skill_level","value":3}] run scoreboard players set @s cause_damage 9000
execute if entity @e[tag=target] if data entity @s SelectedItem.tag.Parameter[{"name":"skill_level","value":4}] run scoreboard players set @s cause_damage 16000
execute if entity @e[tag=target] if data entity @s SelectedItem.tag.Parameter[{"name":"skill_level","value":5}] run scoreboard players set @s cause_damage 25000
