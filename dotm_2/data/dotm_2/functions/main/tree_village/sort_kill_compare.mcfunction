...
execute if score @s boss_3_kill <= @p[tag=boss_fight_3,tag=selected] boss_3_kill run scoreboard players add @s sort 1
tag @p[tag=selected] add compared
tag @p[tag=selected] remove selected
execute if entity @p[tag=boss_fight_3,tag=!compared] as @s run function dotm_2:main/tree_village/sort_kill_compare
