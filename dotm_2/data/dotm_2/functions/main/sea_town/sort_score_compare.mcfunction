...
execute if score @s boss_score <= @p[tag=boss_fight_6,tag=selected] boss_score run scoreboard players add @s sort 1
tag @p[tag=selected] add compared
tag @p[tag=selected] remove selected
execute if entity @p[tag=boss_fight_6,tag=!compared] as @s run function dotm_2:main/sea_town/sort_score_compare
