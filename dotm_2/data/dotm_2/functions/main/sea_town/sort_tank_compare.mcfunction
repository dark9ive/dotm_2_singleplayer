...
execute if score @s boss_6_tank <= @p[tag=boss_fight_6,tag=selected] boss_6_tank run scoreboard players add @s sort 1
tag @p[tag=selected] add compared
tag @p[tag=selected] remove selected
execute if entity @p[tag=boss_fight_6,tag=!compared] as @s run function dotm_2:main/sea_town/sort_tank_compare
